...
const AdminEditRoom = () => {
  ...
  useEffect(() => {
    const getRoom = async () => {
        try {
            const res = await axios.get(`${BASE_URL}/room/${namex}`, {headers: {withCredentials: true}});
  ...
  const EditRoom = async (e) => {
    ...
    try {
        const editRoom = await axios.put(`${BASE_URL}/room/${namex}`,  {headers: {withCredentials: true, "Content-Type": "multipart/form-data"}}, formData);
        if(editRoom) {
