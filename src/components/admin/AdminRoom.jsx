...
const AdminRoom = () => {
  ...
  const searchRoom = async (e, searchId) => {
    e.preventDefault();
    try {
        const searchroom = await axios.get(`${BASE_URL}/room/search/${searchId}`, {headers: {withCredentials: true, "Content-Type": "application/json"}});
  ...
  const deleteRoom = async (name) => {    
    try {
        await axios.delete(`${BASE_URL}/room/${name}`, {headers: {withCredentials: true}});
