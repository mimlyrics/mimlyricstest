...
const EditorPost = () => {
    ...
    async function getVideo() {
      try {
        if(userInfo) {
          ...
          const res = await axios.get(`${BASE_URL}/upload/video/mimlyrics francais`, {headers: {withCredentials: true}});
          console.log("resss ", res.data);
          setVideo(res.data.videos[1].path);
        }
      }
      catch(err) {
          console.log(err);
  ...
  const handlePostSubmit = async (e) => {
    ...
    try {
      ...
      const postVideo = await axios.post(`${BASE_URL}/upload/video`, formData, {headers: {withCredentials: true}});
      if(postVideo) {
  ...
  return (
    <section className=" md:mt-16 visible w-[80vw] md:w-[55vw] bg-zinc-100 md:bg-zinc-200 md:absolute md:top-0 md:left-40 lg:left-60 xl:left-64 mx-2 px-1">
      <div className="mb-8 mt-1 bg-red-200">
        <h1 className="text-2xl text-center bg-indigo-300">Editor DashBard</h1>
      </div>
      {/** Get videos to post */}
